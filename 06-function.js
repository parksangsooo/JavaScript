...
function getNumberTotal() {
    //  ES 특성상 전달되는 모든 매개변수 arguments 함수 객체에 모두 전달
...
//  함수는 객체  --> 변수에 할당되거나 다른 함수의 매개변수로 사용 가능.
function calculator(a, b, func) {    // func --> 계산식 함수를 전달
...
//  함수 보충
console.log("===================== 화살표 표기법");       //  '매개변수 목록 => 구현내용' << 표현식
...
console.log(f2("Parkku"));
const f2_arrow = name => "Hello, " + name;
console.log(f2_arrow('Gang'));
...
const f3_arrow = (a, b) => {
    ...
    블록이 잡히면 반드시 return 문을 넣어줘야 함.*/
    console.log("화살표 함수");
    return a + b;
}
console.log(f3_arrow(10,20));

//  앞서 만든 calculator 콜백 함수에 화살표 함수로 변환
