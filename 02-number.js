...
console.log(Math.min(1, 5, 3, 2, 6, 7), Math.max(1, 5, 3, 2, 6, 7)) // 최소값과 최댓값
console.log(Math.round(3.567), Math.trunc(3.567), Math.floor(3.567))    // floor(버림), trunc(버림)

// casting
console.log("---------------------- casting")
// ES는 주로 문자열로 전달되는 파라미터를 숫자 형태로 변환해야 하는 경우가 많음
// ∴ 데이터 형 변환이 많이 필요함 = parser 계열 함수
...
let v = parseInt("a123.456");       // 변환할 수 없는 형태의 데이터 변환을 하게되면 -> Nan(Not a Number)
